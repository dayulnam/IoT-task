...
19. 예제 5-16 참고2.js
  - Symbol.for을 이용하여 예제 5-15를 수정한 예제이다.
  - Symbol.for은 어디서든 접근 가능하면서 유일무이한 상수를 만들 때 적합하다.

20. 예제 5-17.js
  - 커링 함수는 여러 개의 인자를 받는 함수를 하나의 인자만 받는 함수로 나눠서 순차적으로 호출될 수 있게 체인 형태로 구성한 것이다.
  - 부분 적용 함수와 달리 필요한 인자 개수만큼 함수를 만들어 계속 리턴해주다가 마지막에만 조합해서 리턴해주면 된다.

21. 예제 5-18.js
  - 커링 함수는 인자가 많아질수록 가독성이 떨어지게 된다.
  - ES6에서는 화살표 함수를 통해 같은 내용을 간략화할 수 있다.

22. 예제 5-18 추가1.js
  - 당장 필요한 정보만 받아서 전달하고 또 필요한 정보가 들어오면 전달하는 식으로 하면 결국 마지막 인자가 넘어갈 때까지 함수 실행을 미루는 셈이 되는데 이를 자연실행이라고 한다.
  - 이 예제는 프로젝트 내에서 자주 쓰이는 함수의 매개변수가 항상 비슷하고 일부만 바뀌는 경우에 대한 예제이다.

23. 예제 5-18 추가2.js
  - 고차 함수와 클로저를 사용하여 동적 URL 생성기를 구현한 예시이다.
  - 공통적인 요소는 먼저 기억시켜두고 특정한 값(id)만으로 서버 요청을 수행하는 함수를 만들어 개발 효율성 및 가독성을 향상시킨 것이다.

24. 예제 5-18 추가3.js
  - Flux 아키텍처의 구현체 중 하나인 Redux의 미들웨어를 구현한 예시이다.
  - 예제의 Logger, thunk 두 미들웨어는 공통적으로 store, next, action 순서로 인자를 받는다. 

  #### Chapter 6 예제 요점

1. 예제코드 6-1.js
  - 프로토타입의 기초적인 코드이다. 
  - 이 코드를 추상화하여 그림을 그린 그림6-1 로 프로토타입을 이해할 수 있다.

2. 예제 6-01.js
  - Person이라는 생성자 함수의 prototype에 getName이라는 메서드를 지정한 코드이다.
  - Person의 인스턴스는 __Proto__프로퍼티를 통해 getName을 호출할 수 있다.

3. 예제 6-01 추가코드.js
  - 예제 6-01에 대한 추가적인 코드들이다.(예제라고 따로 표시되어 있지 않은 코드들이다.)
  - getName함수가 실행됨을 이해하고, 함수 내부에서 반환하는 값들의 흐름을 공부한 코드이다.

4. 예제 6-02.js
  - __proto__프로퍼티는 생략 가능하다.
  - 생성자 함수의 prototype에 어떤 메서드나 프로퍼티가 있다면 인스턴스에서도 마치 자신의 것처럼 해당 메서드나 프로퍼티에 접근할 수 있다.

5. 예제 6-03.js
  - prototype 객체 내부의 contructor 프로퍼티를 알아보는 연습이다.
  - 이 프로퍼티는 원래의 생성자 함수를 참조한다.

6. 예제 6-04.js
  - constructor는 읽기 전용 속성이 부여된 예외적인 경우 제외하고 값 변경이 가능하다.

7. 예제 6-05.js
  - constructor의 다양한 접근 방법에 대한 코드들이다.
  - p1~p5까지 모두 Person의 인스턴스이다.

8. 예제 6-06.js
  - 메서드 위에 메서드를 덮어씌운 것을 메서드 오버라이드라고 한다.

9. 예제 6-06 추가코드.js
  - 예제 6-06을 기반으로, 메서드 오버라이딩이 이뤄져 있는 상황에서 prototype에 있는 메서드에 접근하려는 연습이다.
  - name 프로퍼티를 추가하고 call이나 apply로 this가 인스턴스를 바라보도록 바꾸어 준다.

10. 예제 6-07.js
  - 배열에서 배열 매서드 및 객체 메서드를 실행해보는 코드이다.

11. 예제 6-08.js
  - 어떤 데이터의 __proto__프로퍼티 내부에 다시 __proto__프로퍼티가 연쇄적으로 이어진 것을 프로토타입 체인이라고 한다.
  - 또한, 체인을 따라가며 검색하는 것을 프로토타입 체이닝이라고 한다.

12. 예제 6-09.js
  - 코드 실행 결과, 11~16번째 줄의 각 데이터마다 getEntries를 실행하면 모든 데이터가 오류 없이 결과를 반환한다.
  - 어느 데이터 타입이건 거의 무조건 프로토타입 체이닝을 통해 getEntries 메서드에 접근할 수 있기 때문이다.
  - 따라서, 객체만을 대상으로 동작하는 객체 전용 메서드들은 부득이 Object.prototype이 아닌 Object에 스태틱 메서드로 부여할 수 밖에 없었다.

13. 예제 6-10.js
  - 대각선의 __proto__를 연결하는 방법은 __proto__가 가리키는 대상인 생성자 함수의 prototype이 연결하고자 하는 상위 생성자 함수의 인스턴스를 바라보게끔 한다.

  #### Chapter 7 예제 요점

1. 예제 7-01.js
  - 클래스 관점에서 바라본 프로토타입 시스템을 파악해보는 코드이다.
  - 프로토타입에 클래스 개념을 적용할 수 있다.

2. 예제 7-02.js
  - ES5까지 자바스크립트에 클래스가 없기 때문에 자바스크립트에서 클래스 상속을 구현하는 것은 결국 프로토타입 체이닝 연결을 잘 한것이다.

3. 예제 7-03.js
  - length 프로퍼티는 내장객체인 배열 인스턴스이다.
  - length 프로퍼티는 configurable 속성이 false이기 때문에 삭제가 불가능하지만, Grade 클래스의 인스턴스는 배열 메서드를 상속하지만 기본적으로 일반 객체의 성질을 그대로 지니므로 삭제가 가능해진다. 이것이 문제가 된다.

4. 예제 7-04.js
  - Grade.prototype에 요소를 포함하는 배열을 매칭시킨 것이다.
  - 본 코드는 클래스에 있는 값이 인스턴스의 동작에 영향을 준 경우이다.

5. 예제 7-05.js
  - getArea는 넓이를 구하는 메서드이다.
  - Rectangle, Square 클래스에 width라는 프로퍼티가 공통이며 getArea는 내용은 다르지만 비슷하다.

6. 예제 7-06.js
  - 예제 7-05를 기반으로, Square에서 width 프로퍼티만 쓰지 않고 height 프로퍼티에 width값을 부여하는 형태로 만들어 주었다.

7. 예제 7-07.js
  - 예제 7-06를 기반으로, getArea 메서드는 상위 클래스에서만 정의하고 하위 클래스에서는 해당 메서드를 상속하면서 height 대신 width를 넣어주었다.
  - Rectangle을 상속하는 Square 클래스 형태가 되었다.

8. 예제 7-08.js
  - 클래스 상속 및 추상화 방법 첫 번째로, 인스턴스 생성 후 프로퍼티 제거하는 방법이다.
...
10. 예제 7-09.js
  - 클래스 상속 및 추상화 방법 두 번째로, 예제 7-08 추가코드와 같이 빈 함수를 활용하는 방법이다.
...
11. 예제 7-10.js
  - 클래스 상속 및 추상화 방법 세 번째로, ES5에서 도입된 Object.create를 활용하는 방법이다.
...
14. 예제 7-13.js
  - 클래스 상속 및 추상화 방법 중 Object.create 활용하는 것의 완성본이다.
  - 마찬가지로 SubClass.prototype.constructor가 원래의 SubClass를 바라보도록 해주었다.
